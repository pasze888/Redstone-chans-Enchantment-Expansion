# 附魔效果组件体系 — Redstone-chan's Enchantment Expansion

记录已验证（编译通过 / runData 通过 / 源码核对）的附魔效果组件 API 事实与本仓库确立的架构，供后续会话复用。
各批次迁移与行为变更见 `enchantment-migrations.md`，附魔 JSON 迁 datagen 见 `enchantment-datagen.md`，
光环与 mcfunction 迁移的运行期事实见 `enchantment-runtime-effects.md`，附魔全表见 `enchantments.md`；
构建 / 环境坑见 `../troubleshooting.md`，AI 协作与跨会话沉淀坑见 `../ai/gotchas.md`。
...
- 挖掘类效果的执行顺序从"订阅者注册顺序（不确定）"改为固定：连锁急迫 → 自动熔炼 → 概率掉落 → 连锁砍树 → 区域挖掘。
  - **补记（2026-09-24，掉落处理迁到 `BlockDropsEvent` 后）**：现顺序为 连锁急迫（破坏前）→ 精通采集（复制掉落）→
    自动熔炼（启用时跳过下一步）→ 地质学 / 点石成金（追加掉落）→ 连锁砍树 → 区域挖掘。自动熔炼与"地质学 / 点石成金"
    互斥关系不变，精通采集被特意提到自动熔炼之前，使两者同时附魔时复制出的原矿会一起进熔炼。详见
    `enchantment-migrations.md` 的「挖掘加成掉落批次」与「auto_smelt 改用 BlockDropsEvent」两节。
